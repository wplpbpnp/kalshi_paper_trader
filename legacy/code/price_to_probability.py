...
class PriceToProbability:
    ...
    def get_surface_matrix(self, for_plot=False):
        ...
        for (x_bucket, time_bucket), data in self.surface_data.items():
            total = data["yes"] + data["no"]
            if total > 0:
                matrix[x_bucket, time_bucket] = data["yes"] / total
                counts[x_bucket, time_bucket] = total
    ...
    def print_ev_surface(self):
        ...
        # Calculate composite scores (now includes recency)
        kelly_normalized = np.where(np.isinf(kelly_matrix), 1.0, np.clip(kelly_matrix, 0, 1))
        confidence = counts / (counts + self.confidence_k)
        composite = ev_matrix * np.power(trade_freq, self.trade_exponent) * kelly_normalized * confidence * recency_matrix
        ...
        print(f"{x_header:<18} {'Time':<12} {'EV':>10} {'P(win)':>10} {'Kelly':>10} {'Samples':>8} {'Trades':>8} {'Recency':>8} {'Score':>10}")
        print("-" * 116)

        for opp in opportunities[:10]:
            if opp["composite"] > 0:
                kelly_str = "MAX" if opp['kelly'] == float('inf') else f"{opp['kelly']*100:.1f}%"
                print(f"{opp['x_label']:<18} {opp['time']:<12} {opp['ev']:>+9.3f}% {opp['p_win']*100:>9.1f}% {kelly_str:>10} {opp['n']:>8} {opp['trades']:>8} {opp['recency']:>7.0%} {opp['composite']:>+9.2f}")

        # NO opportunities (negative YES EV = positive NO EV)
        print("\nTOP NO OPPORTUNITIES (buy NO / sell YES)")
        print("-" * 116)
        if self.mode == 'prediction':
            print(f"{'YES @':<10} {'NO @':<10} {'Time':<12} {'EV(NO)':>10} {'P(NO)':>10} {'Kelly':>10} {'Samples':>8} {'Trades':>8} {'Recency':>8} {'Score':>10}")
        else:
            print(f"{x_header:<18} {'Time':<12} {'EV(short)':>10} {'P(lose)':>10} {'Kelly':>10} {'Samples':>8} {'Trades':>8} {'Recency':>8} {'Score':>10}")
        print("-" * 116)
        ...
        for opp in opportunities:
            ...
            if opp['ev'] < 0 and p_no > 0:  # Negative YES EV means positive NO EV
                ...
                kelly_norm = 1.0 if kelly_no == float('inf') else min(1.0, max(0, kelly_no))
                confidence = opp['n'] / (opp['n'] + self.confidence_k)
                composite_no = ev_no * (opp['trades'] ** self.trade_exponent) * kelly_norm * confidence * opp['recency']
        ...
        for opp in no_opportunities[:10]:
            ...
            if self.mode == 'prediction':
                print(f"{opp['yes_label']:<10} {opp['no_label']:<10} {opp['time']:<12} {opp['ev_no']:>+9.3f}% {opp['p_no']*100:>9.1f}% {kelly_str:>10} {opp['n']:>8} {opp['trades']:>8} {opp['recency']:>7.0%} {opp['composite_no']:>+9.2f}")
            else:
                print(f"{opp['yes_label']:<18} {opp['time']:<12} {opp['ev_no']:>+9.3f}% {opp['p_no']*100:>9.1f}% {kelly_str:>10} {opp['n']:>8} {opp['trades']:>8} {opp['recency']:>7.0%} {opp['composite_no']:>+9.2f}")
    ...
    def export_decision_table(self, output_path, min_composite=0.1, fractional_kelly=0.25):
        ...
        kelly_normalized = np.where(np.isinf(kelly_matrix), 1.0, np.clip(kelly_matrix, 0, 1))
        confidence = counts / (counts + self.confidence_k)
        composite_yes = ev_matrix * np.power(trade_freq, self.trade_exponent) * kelly_normalized * confidence * recency_matrix
        ...
        for i in range(len(x_labels)):
            for j in range(len(time_labels)):
                ...
                if ev < 0:
                    ...
                    kelly_no_norm = 1.0 if kelly_no == float('inf') else min(1.0, max(0, kelly_no))
                    composite_no = ev_no * (trade_freq[i, j] ** self.trade_exponent) * kelly_no_norm * confidence[i, j] * recency_matrix[i, j]
    ...
    def plot_ev_surface(self, output_path=None):
        ...
        # Recency heatmap
        recency_masked = np.ma.masked_where((counts < 10) | (recency_matrix < 0.01), recency_matrix)
        im3 = ax3.imshow(recency_masked, aspect='auto', cmap='YlOrRd', vmin=0, vmax=1)
        ...
        ax3.set_ylabel(y_label)
        ax3.set_title(f'Recency Score (half-life={self.recency_half_life} windows)\n1.0 = recent, 0.5 = half-life ago')
        plt.colorbar(im3, ax=ax3, label='Recency')

        # Add text annotations for recency
        for i in range(len(x_labels)):
            for j in range(len(time_labels)):
                if counts[i, j] >= 10 and recency_matrix[i, j] >= 0.01:
                    r = recency_matrix[i, j]
                    text = f'{r:.0%}'
                    color = 'white' if r > 0.5 else 'black'
                    ax3.text(j, i, text, ha='center', va='center', color=color, fontsize=7)

        # Composite score: combines EV, Kelly, trade frequency, confidence, and recency
        # Formula: |EV| × trades^exp × kelly_factor × confidence × recency
        # Sign indicates direction: positive = buy YES, negative = buy NO
        # - EV: expected return per trade (%)
        # - trades^exp: trade frequency with configurable exponent (0.5=sqrt, 1.0=linear)
        # - kelly_factor: normalized Kelly (0-1), calculated for appropriate side
        # - confidence: sample_count / (sample_count + k), saturates toward 1
        # - recency: decay-weighted average of trade ages

        confidence = counts / (counts + self.confidence_k)

        # Calculate composite for both YES and NO sides
        # For YES side (EV > 0): use kelly_YES
        # For NO side (EV < 0): calculate kelly_NO and use |EV|
        ...
        for i in range(ev_matrix.shape[0]):
            for j in range(ev_matrix.shape[1]):
                ...
                tf = trade_freq[i, j]
                conf = confidence[i, j]
                rec = recency_matrix[i, j]

                if ev >= 0:
                    # YES side - use YES kelly
                    kelly_norm = 1.0 if np.isinf(k) else min(1.0, max(0, k))
                    composite[i, j] = ev * (tf ** self.trade_exponent) * kelly_norm * conf * rec
                else:
                    # NO side - calculate NO kelly from surface data
                    key = (i, j)
                    data = self.surface_data.get(key, {})
                    p_no = 1 - (p_win_matrix[i, j] if p_win_matrix[i, j] > 0 else 0.5)

                    # avg_win for NO = avg_loss for YES, avg_loss for NO = avg_win for YES
                    avg_win_no = np.mean(data.get("loss_magnitudes", [0])) if data.get("loss_magnitudes") else 0
                    avg_loss_no = np.mean(data.get("win_magnitudes", [0])) if data.get("win_magnitudes") else 0
                    ...
                    kelly_norm = min(1.0, kelly_no)
                    ev_no = -ev  # Flip to positive
                    # Store as negative to indicate NO side
                    composite[i, j] = -ev_no * (tf ** self.trade_exponent) * kelly_norm * conf * rec
        ...
        im4 = ax4.imshow(composite_masked, aspect='auto', cmap='RdYlGn', vmin=-max_abs_composite, vmax=max_abs_composite)
        ax4.set_xticks(range(len(plot_time_labels)))
        ax4.set_xticklabels(plot_time_labels, rotation=45, ha='right', fontsize=8)
        ax4.set_yticks(range(len(x_labels)))
        ax4.set_yticklabels(x_labels)
        ax4.set_xlabel('Time Elapsed')
        ax4.set_ylabel(y_label)
        exp_str = "√" if self.trade_exponent == 0.5 else f"^{self.trade_exponent}"
        if self.mode == 'prediction':
            ax4.set_title(f'Composite Score (Green=YES, Red=NO)\n|EV| × trades{exp_str} × kelly × conf × recency')
        else:
            ax4.set_title(f'Composite Score\nEV × trades{exp_str} × kelly × conf × recency')
        plt.colorbar(im4, ax=ax4, label='Score (+YES / -NO)')
            ...
            for j in range(len(time_labels)):
                if counts[i, j] >= 10:
                    ...
                    if abs(score) >= 0.001:
                        ...
                        ax4.text(j, i, text, ha='center', va='center', color=color, fontsize=7)
    ...
    def export_llm_analysis(self, filepath):
        ...
        for (dist_bucket, time_bucket), data in self.surface_data.items():
            ...
            # Composite score (includes recency)
            kelly_norm = 1.0 if kelly == "MAX" else (kelly if isinstance(kelly, (int, float)) else 0)
            confidence = total_samples / (total_samples + self.confidence_k)
            composite = ev_pct * (trade_freq ** self.trade_exponent) * kelly_norm * confidence * recency
        ...
        output = {
            "metadata": {
                ...
                "composite_formula": f"EV × trades^{self.trade_exponent} × kelly × confidence × recency",
                "confidence_k": self.confidence_k,
                "recency_half_life": self.recency_half_life,
                "trade_exponent": self.trade_exponent,
                "created": datetime.now().isoformat()
            ...
            "interpretation_guide": {
                ...
                "recency": "How recent the trades were (100% = all recent, 50% = avg age equals half-life, lower = older)",
                "composite_score": "Combined score ranking opportunities by return potential, confidence, and recency"
            },
...
def interactive_query(p2p):
    ...
    while True:
        try:
            ...
            # Calculate composite score (includes recency)
            kelly_norm = 1.0 if ev_data['kelly'] == float('inf') else min(1.0, max(0, ev_data['kelly']))
            confidence = ev_data['n'] / (ev_data['n'] + p2p.confidence_k)
            recency = ev_data['recency']
            composite = ev_data['ev'] * (ev_data['trades'] ** p2p.trade_exponent) * kelly_norm * confidence * recency
            ...
            print(f"  Recency: {recency:.0%} | Composite Score: {composite:+.3f}")
